...
@onready var defaultItem = preload("res://Scenes/UI/Tasks/TaskItem.tscn");
...
func _load_tasks(tasks: Dictionary, tier: int = 1):
	...
	var task_path: String = TaskDatabase.get_task_path(limitNumber, tier);
	while (solvedTaskIndexes.find(limitNumber) != -1):
		randomize();
		limitNumber = randi_range(1, tasks.size());
		task_path = TaskDatabase.get_task_path(limitNumber, tier);
	loadLimitAsJSON(task_path);
	solvedTaskIndexes.append(limitNumber);
